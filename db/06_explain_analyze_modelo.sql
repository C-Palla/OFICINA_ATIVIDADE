...
-- Use este arquivo para coletar evidências da Entrega 5.
-- Passo recomendado:
-- 1) Rode 02_ddl.sql e 03_dados.sql.
-- 2) Rode este arquivo ANTES de criar índices adicionais.
-- 3) Rode 05_indices.sql.
-- 4) Rode este arquivo novamente.
-- 5) Cole as saídas no relatório docs/relatorio_explain_analyze_modelo.md.
...
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
SELECT TO_CHAR(DATE_TRUNC('month', a.data_conclusao), 'YYYY-MM') AS mes,
...
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
SELECT nome, fornecedor, quantidade_atual, quantidade_minima,
...
WITH avaliacoes_por_funcionario AS (
  SELECT DISTINCT f.id_funcionario, f.nome, av.id_avaliacao, av.nota
  FROM funcionario f
  JOIN item_servico isv ON isv.id_funcionario = f.id_funcionario
  JOIN agendamento a ON a.id_agendamento = isv.id_agendamento
